...
const Item = (props) => {
  const { img, name, currentPrice, startDate, id } = props;
  const navigate = useNavigate();
  return (
    // <Link to=`/productdetail/${id}`>
    // <div onClick={navigate(`/productdetail/${id}`)}>
    <div
      className=" border-solid border-[4px] rounded-[12px] border-[#52AB98] p-4 mt-4 h-[500px] cursor-pointer "
    ...
    >
      ...
      <div className="h-[100px]">
        <p
          className=" text-2xl text-[#000000] overflow-hidden line-clamp-3 text-ellipsis hover:text-[red] cursor-pointer"
          onClick={() => navigate(`/productdetail/${id}`)}
        >
          {name}
        </p>
