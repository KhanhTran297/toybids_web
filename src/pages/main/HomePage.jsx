...
import {
  getListAuctioningApi,
  getListUpcomingAuctionApi,
} from "../../api/auction";
...
const HomePage = () => {
  ...
  const { data: listUpcomingAuction } = useQuery({
    queryKey: ["listUpcomingAuction", currentTime],
    queryFn: getListUpcomingAuctionApi,
    enabled: !!currentTime,
  });
  const { data: listAuctioning } = useQuery({
    queryKey: ["listAuctioning", currentTime],
    queryFn: getListAuctioningApi,
    enabled: !!currentTime,
  });

  return (
    <div className=" w-full mt-[100px]  flex justify-center items-center">
      <div className="flex flex-col gap-12 m-4 w-[80%]">
        ...
        <div className="flex flex-col gap-2 tren ">
          ...
          <div className=" grid grid-flow-row grid-cols-[25%_25%_25%_25%] gap-2">
            {listUpcomingAuction?.data?.content
              ?.slice(0, 8)
              .map((auction, index) => (
                <div key={index}>
                  <Item
        ...
        <div className="flex flex-col gap-2 tren ">
          <div className="flex flex-row justify-between ">
            <div className="flex flex-row gap-2">
              ...
              <div className=" text-[#2B6777] text-4xl font-bold">
                Auction is going on
              </div>
            </div>
          </div>
          <div className=" grid grid-flow-row grid-cols-[25%_25%_25%_25%] gap-2">
            {listAuctioning?.data?.content
              ?.slice(0, 8)
              .map((auction, index) => (
                <div key={index}>
                  <Item
